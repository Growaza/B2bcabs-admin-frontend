...
function EditBookingConfirm(props) {
  ...
  React.useEffect(async () => {
    const data = await myApi.get(`/api/v1/booking/confirmbooking/${id}/`);
  ...
  const updateBooking = async () => {
    ...
    try {
      const updateData = await myApi.put(`/api/v1/booking/confirmbooking/${id}/`, data);
      setNotify({
        isOpen: true,
        message: "Booking Updated",
        type: "success",
      });
      return navigate(`/cancel-bookings/`);
    } catch (e) {
      setNotify({
        isOpen: true,
